...
std::string g_output_mesh_name = "output";
...
{
	std::vector<COMTMesh::CFace*> removed_faces;
  ...
  case '!':
	  ...
	  break;
  case '@':
	  OT_Newton_all(mesh, D, step_length, configure.m_nearest, threshold);
	  break;
  case 'm':
  ...
  case 'W':
	{	  
	  for (COMTMesh::MeshVertexIterator vit(&mesh); !vit.end(); ++vit)
	  {
		  ...
		  COMTMesh::CVertex* pW = res_mesh.idVertex(pV->id());

		  if (pV->id() <= 0)
		  {
			  continue;
		  }
		  double u = pV->uv()[0], v = pV->uv()[1];
		  pW->uv()[0] = u + 0.5;
		  pW->uv()[1] = v + 0.5;
		  pW->point()[0] += 0.5;
		  pW->point()[1] += 0.5;
	  ...
	  res_mesh.write(g_output_mesh_name + ".m");
	  std::cout << "Output to " << g_output_mesh_name + ".m" << std::endl;
	  break;
	}
  case 'q':
	  exit(0);
...
int main( int argc, char * argv[] )
{
	...
	for( int i = 1; i < argc; i ++ )
	{
		...
		if( strcmp( argv[i], "-target" ) == 0 )
		{
			...
			name = mesh_name;
			g_output_mesh_name = mesh_name.substr(0, mesh_name.rfind("."));
			...
			if( strutil::endsWith( mesh_name, ".obj" ) )
			{
				...
				res_mesh.read(mesh_name.c_str());
				for (COMTMesh::MeshVertexIterator viter(&res_mesh); !viter.end(); viter++)
				{
					COMTMesh::CVertex* pv = *viter;
					COMTMesh::CHalfEdge* ph = res_mesh.vertexHalfedge(pv);
					pv->uv() = ph->uv();
				}
			}
		...
		{
			std::string output_mesh_name(argv[++i]);
			g_output_mesh_name = output_mesh_name.substr(0, output_mesh_name.rfind("."));
		}
		...
		if (strcmp(argv[i], "-source") == 0)
		{
			...
			for (std::vector<COMTMesh::CHalfEdge*>::iterator iter = pL->halfedges().begin(); iter != pL->halfedges().end(); iter++)
			{
				...
				pts.push_back(CPoint2(pv->point()[0], pv->point()[1]));
			}
	...
	OT_Initialize<COMTMesh>(mesh, D, configure.m_nearest, scale);

...
namespace DartLib
{
	...
	void COMTDynamicMesh<V, E, F, H>::__calculate_hessian(Eigen::SparseMatrix<double> &m_hessian)
	{
		...
		for (MeshEdgeIterator eiter(this); !eiter.end(); eiter++)
		{
			...
			if (this->_infinite(pv0) || this->_infinite(pv1))
				continue;
	...
	void _OT_damping(COMTMesh &mesh, CPolygon2D &D, double step, const bool nearest)
	{
		...
		while (true)
		{
			...
			if (mesh._Lawson_edge_swap(nearest))
				break;
			// roll back
	...
	// 自动计算到阈值
	template <class COMTMesh>
	void OT_Newton_all(COMTMesh &mesh, CPolygon2D &D, const double step_length, const bool nearest, const double thresthold)
	{
		double error = mesh._error();
		while (error >= thresthold)
		{
			mesh._update_direction(D);
			_OT_damping<COMTMesh>(mesh, D, step_length, nearest);
			error = mesh._error();
	...
	template <class COMTMesh>
	void OT_Initialize(COMTMesh &mesh, CPolygon2D &D, const bool nearest, const double scale)
	{
		int idx = 0;
		for (typename COMTMesh::MeshVertexIterator viter(&mesh); !viter.end(); viter++)
		{
		...
		for (typename COMTMesh::MeshVertexIterator viter(&mesh); !viter.end(); viter++)
		{
			typename COMTMesh::CVertex *pV = *viter;
			// 分段函数
			if (pV->texture_area() < 0.0000001)
			{
				pV->target_area() *= scale;
			}
			// 线性权重
			// pV->target_area() *= (scale + (1 - scale) * pV->texture_area());

			// 开方权重
			// pV->target_area() *= (scale + (1 - scale) * std::sqrt(pV->texture_area()));
		}
		...
		for (typename COMTMesh::MeshVertexIterator viter(&mesh); !viter.end(); viter++)
		{
			typename COMTMesh::CVertex *pv = *viter;
			s += pv->target_area();
			// printf("vertex %d area %g\n", pv->id(), pv->target_area());
		}
		double ds = D.area();
		printf("surface area: %g, source domain area: %g\n", s, ds);
		for (typename COMTMesh::MeshVertexIterator viter(&mesh); !viter.end(); viter++)
		{
			typename COMTMesh::CVertex *pv = *viter;
			pv->target_area() *= (ds / s);
